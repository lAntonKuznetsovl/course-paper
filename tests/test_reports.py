...
import pandas as pd
import json
from src.reports import spending_by_category, average_cost_amount, writing_report
...
def test_spending_by_category_with_incorrect_category(capsys, test_df):
    ...
    result = capsys.readouterr()
    assert (
            result.out == 'Неверно введена категория\nФормирование отчёта завершено\n'
    )
...
def test_average_cost_amount_with_incorrect_date_format(capsys, test_df):
    ...
    result = capsys.readouterr()
    assert (
            result.out == 'Введён не верный формат даты\n'
    )
...
def test_writing_report():
    ...
    filename = "test_data"
    test_dict = {"Запись 1": ["тест1", "тест2"],
                 "Запись 2": ["тест1", "тест2"]}
